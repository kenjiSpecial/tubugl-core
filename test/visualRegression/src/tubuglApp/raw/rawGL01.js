...
export default class App {
	...
	createProgram() {
		...
		let fragmentShader1 = webGLShader(
			this.gl,
			this.gl.FRAGMENT_SHADER,
			fragmentShaderSrc(0.0, 0.0, 1.0)
		);
		this._program1 = this.initProgram(vertexShader, fragmentShader1);
		//
		this._positionAttribLocation1 = this.gl.getAttribLocation(
			this._program1,
			'a_position'
		);
		...
		this.gl.bufferData(
			this.gl.ARRAY_BUFFER,
			new Float32Array(positions2).buffer,
			this.gl.STATIC_DRAW
		);
